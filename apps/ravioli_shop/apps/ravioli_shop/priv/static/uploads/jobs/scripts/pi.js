...
var calcPi = function calcPi() {
  ...
  console.log(hit);
  console.groupEnd();
  channel.push("result", { hit: hit, round: round });
};

var channel = _socket2.default.channel("pi:monte", {});
channel.on("calculate", function (x) {
  calcPi();
...
var socket = new _phoenix.Socket("http://localhost:4000/socket", { params: { token: window.userToken } });
...
var channel = socket.channel("pi:monte", {});
channel.join().receive("ok", function (resp) {
  console.log("Joined successfully", resp);
}).receive("error", function (resp) {
  console.log("Unable to join", resp);
